var data = `ayitmcjvlhedbsyoqfzukjpxwt
...
set.forEach(code => {
    ...
    while (i--) {
        // alert(code.charAt(i));
        if(chars[code.charAt(i)] == undefined){
            chars[code.charAt(i)] = 1;
        }else{
            chars[code.charAt(i)]++;
        }
      }
...
set.forEach(code => {
    ...
    //for every code, go through each code
    set.forEach(ocode => {
        var charDiff = 0;
        var count = code.length;
        //iterate through the characters, count the number of differences for this string
        while(count--){
            if(code.charAt(count) != ocode.charAt(count)){
                charDiff++;
            }
        }
        if(charDiff == 1){
            console.log("code: " + code  + " ocode: " +ocode);
        }
    })
})