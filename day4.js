var data = `[1518-10-31 00:58] wakes up
...
// [1518-06-06 23:56] Guard #3271 begins shift
var reg = /\[(.+?)\] ((Guard #(\d{1,5}) begins shift)|(wakes up|falls asleep))/
...
//Now that we're sorted, lets fill out the rest of the object
records.forEach(r=>{
        if(r.matchData[4] != undefined){
            //new guard
            lastGuard = r.matchData[4];
            r.guard = r.matchData[4];
            r.awake = true;
        }else{
            r.guard = lastGuard;
            r.awake = (r.matchData[5] == 'falls asleep') ? false : true;
        }
        delete r.matchData;

        // console.log(`'${r.guard}','${r.date.toLocaleString()}', '${r.awake}'`);
})

var guards = {}

for(i = 0; i < records.length; i++) {
    if(records[i].date.getHours() == 00 &&
        records[i].awake == false
    ) {
        ...
        for(t = 0; t < 59; t++){
            ...
            if(guards[records[i].guard].sleep[t] == undefined) {
                guards[records[i].guard].sleep[t] = 0;
            }

            //set the minutes asleep 
            if(t >= timeAsleep && t < (timeAsleep + durationSleep)) {
                //Add a minute of sleep
                guards[records[i].guard].sleep[t]++;
            }
        }
    }
}
